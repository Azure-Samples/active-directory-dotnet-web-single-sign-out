...
#> 
   ...
   Write-Host "Creating the AAD application (WebApp-DistributedSignOut-DotNet)"
   # Get a 2 years application key for the service Application
   $pw = ComputePassword
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInYears 2 -pw $pw
   $serviceAppKey = $pw
   $serviceAadApplication = New-AzureADApplication -DisplayName "WebApp-DistributedSignOut-DotNet" `
                                                   -HomePage "https://localhost:44308/" `
                                                   -LogoutUrl "https://localhost:44308/Account/EndSession" `
                                                   -ReplyUrls "https://localhost:44308/" `
                                                   -IdentifierUris "https://$tenantName/WebApp-DistributedSignOut-DotNet" `
   ...
   Write-Host "Done creating the service application (WebApp-DistributedSignOut-DotNet)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>WebApp-DistributedSignOut-DotNet</a></td></tr>" -Path createdApps.html
